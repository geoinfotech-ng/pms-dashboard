const FuelData = (() => {
  ...
  function getPriceBandDistribution() {
    const bands = Config.PRICE_BANDS.map(b => ({ ...b, count: 0 }));
    _data.forEach(d => {
      const idx = Config.getPriceBandIndex(d.price);
      if (idx >= 0 && idx < bands.length) bands[idx].count++;
    });
    return bands.filter(b => b.count > 0);
  }

  return { load, getPrice, getStateData, getStats, getByPrice, getRank, getZoneAverages, getPriceBandDistribution, get data() { return _data; } };
})();
