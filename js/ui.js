const UI = (() => {
  ...
  function doPDF() {
    ...
    doc.text('Geoinfotech', M + 24, y + 9);
    doc.setFont('helvetica', 'normal'); doc.setFontSize(9); doc.setTextColor(120);
    doc.text('Geospatial Intelligence & Technology', M + 24, y + 15);
    doc.setTextColor(0);
    ...
    doc.text(`Data Period: ${stats.date}  |  Source: NBS PMS Price Watch  |  Generated: ${new Date().toLocaleDateString('en-NG')}`, W / 2, y, { align: 'center' });
    doc.setTextColor(0);
    y += 14;
    ...
    doc.setFillColor(235, 235, 235); doc.rect(M, y - 5.5, CW, rh, 'F');
    doc.setFont('helvetica', 'bold'); doc.setFontSize(11);
    doc.text('Metric', col1 + 2, y); doc.text(a.state, col2 + 2, y); doc.text(b.state, col3 + 2, y);
    y += rh + 2;
    ...
    rows.forEach((row, i) => {
      if (i % 2 === 0) { doc.setFillColor(248, 248, 248); doc.rect(M, y - 5.5, CW, rh, 'F'); }
      doc.text(row[0], col1 + 2, y); doc.text(row[1], col2 + 2, y); doc.text(row[2], col3 + 2, y);
      y += rh + 1;
    });
    y += 8;
    ...
    for (let p = 1; p <= pc; p++) {
      doc.setPage(p);
      doc.setDrawColor(200); doc.line(M, 274, W - M, 274);
      doc.setFontSize(7.5); doc.setFont('helvetica', 'normal'); doc.setTextColor(120);
      doc.text(`${c.name} | ${c.address}`, W / 2, 278, { align: 'center' });
      doc.text(`Email: ${c.email1} | ${c.email2}   Phone: ${c.phone1} | ${c.phone2}`, W / 2, 282, { align: 'center' });
      doc.text(`Page ${p} of ${pc}`, W / 2, 287, { align: 'center' });
      doc.setTextColor(0);
    }
